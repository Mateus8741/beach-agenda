...
const navItems: NavItem[] = [
  { id: 1, icon: 'home-outline', label: 'Home', active: true },
  { id: 2, icon: 'search-outline', label: 'Search' },
  { id: 3, icon: 'add-circle-outline', label: 'Bookings' },
  { id: 4, icon: 'calendar-outline', label: 'Calendar' },
  { id: 5, icon: 'person-outline', label: 'Profile' },
];
          ...
          {navItems.map((item) => (
            <TouchableOpacity key={item.id} className="items-center">
              <Ionicons
                ...
                size={24}
                color={item.active ? '#f97316' : '#666'}
              />
              <Text className={`text-xs ${item.active ? 'text-orange-500' : 'text-gray-600'}`}>
                {item.label}
              </Text>
            </TouchableOpacity>
          ))}
