...
export function SportSelect() {
  const { setSport, selectedSport } = useBookingStore();

  return (
    <View className="flex-row gap-3 py-4">
      {sports.map((sport) => {
        const isSelected = sport.id.toString() === selectedSport?.id;

        return (
          <TouchableOpacity
            key={sport.id}
            onPress={() => {
              setSport({ id: sport.id.toString(), name: sport.name });
            }}
            className={
              isSelected
            ...
            <Text
              className={
                isSelected ? 'text-center text-xs text-white' : 'text-center text-xs text-gray-600'
              }>
              {sport.name}
            </Text>
          </TouchableOpacity>
